...
from evals.elsuite.canvassing.solvers.prompts import (
    CANVASSER_MSG,
    COT_MESSAGE,
    EXTRACT_MESSAGE,
    TIME_REMINDER_MSG,
...
class LMCanvasserSolver(Solver):
    def __init__(
        self,
    ...
    def _solve(self, task_state: TaskState, **kwargs) -> SolverResult:
        ...
        # this is start of conversation, otherwise this is included in memory
        if self.lm_system_prompt:
            max_turns_or_time = task_state.current_state["max_turns_or_time"]

            content = CANVASSER_MSG.format(duration=max_turns_or_time)
